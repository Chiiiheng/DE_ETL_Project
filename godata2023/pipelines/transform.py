...
def trade_volume_aggregator(df):
    ...
    window_open = Window.partitionBy('trade_date','symbol').orderBy('trade_timestamp')
    window_close = Window.partitionBy('trade_date','symbol').orderBy(desc('trade_timestamp'))
    ...
    daily_trade = daily_trade.withColumn("partition",col("trade_date").cast('string'))
